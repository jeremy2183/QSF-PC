...
function widthdraw() 
	{
		//alert('fee='+$('#fee').val()+'reached='+$('#reached').val());
		/*
		if($('#amount').val()=='' || $('#accountName').val()=='' || $('#account').val()=='' || $('#widthdrawpassword').val()=='' || $('#bankid').val()=='' )
		{
			//alert('请检查资料是否填写完毕');
...
function register()
{
	...
	if($('#name').val()=='' || $('#nickname').val()=='' )
	{
		//alert('请输入帐号与昵称');
		$('#nickname').val('');
		var info = {'status':0, 'title':'', 'text1':'请输入帐号与昵称', 'text2':'' };
		msg(info);
	}
